...
----------浮动---float：left  可转换行内元素
	 清除浮动方法
	 1 额外标签法 最后面标签添加clear：both 清除浮动
	 2 父亲添加overflow:hidden/auto/scroll 方法
	 3 .clearfix:after{}伪元素清除浮动
	 4：双伪元素清除浮动
--------position：fixed absolute  relative
	  z-index
	  display:none隐藏
	  display:black  块级别元素 显示元素
	  visibility：visible hidden
	 overflow：scroll  hidden   auto
---------鼠标样式--
	 cursor:default,text,pointer,move
	 outline:none 	取消轮廓线
	 textarea    文本框输入  resize：none 防止文本框拖拽
	      white-space: nowrap;
